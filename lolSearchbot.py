...
bottoken = 'ODQzMTMzODU4OTI5MzExNzQ0.YJ_bmA.XTKxmoEFHQg2u7LK1FiHh1s9YRA'
...
async def on_message(message): # on_message() event : when the bot has recieved a message
    ...
    if message.content.startswith("동준아 도움말") or message.content.startswith("!도움말"):
        embed = discord.Embed(title="명령어 사용방법!", description="동준아 도움말 (소환사 이름 - 띄어쓰기 붙여쓰기 상관없습니다)", color=0x5CD1E5)
        embed.set_footer(text='Developer. 안녕? AIMGRO#1111',
    ...
    if message.content.startswith("동준아 롤전적"):
        try:
            if len(message.content.split(" ")) == 1:
                ...
                embed.add_field(name="Summoner name not entered",
                                value="To use command 동준아 롤전적 : 동준아 롤전적 (Summoner Nickname)", inline=False)
                embed.set_footer(text='Service provided by Hoplin.',
                                 icon_url='https://i.pinimg.com/originals/78/2d/1e/782d1e453860ff0a439eb27e6ea2c03a.jpg')
